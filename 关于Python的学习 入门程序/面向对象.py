...
# class 子类名(父类名1,父类名2...):
#   
# 创建子类方法时 可以调用多个父类方法 引用方法和单继承一致
# 如果不同的父类中存在同名的方法 子类对象在调用方法时 会调用哪一个父类方法的方法？
# 提示: 开发时 应该尽量避免这种容易产生混淆的情况  如果父类之间存在同名的属性或者方法时 应该避免使用多继承
# Python的向上查找方法在赋值语句的时候不会适用 比如用对象名调用类属性时会成功 但是当对象名.类属性=赋值 
# 时由于在对象属性中没有查找到 便会重新创建一个对象属性 而类属性不会被复制改变 此时在类中便出现了两个名称一样的属性名
# 一个是类属性 一个是对象属性Python的向上查找方法在赋值语句的时候不会适用 比如用对象名调用类属性时会成功
# 但是当对象名.类属性=赋值 时由于在对象属性中没有查找到 便会重新创建一个对象属性 而类属性不会被复制改变 此时在类中便出现了两个名称一样的属性名 一个是类属性 一个是对象属性
# #
#Python中的方法搜索顺序 ---MRO(了解)

#print(C.__mro__)
# 在搜索方法时 是按照__mro__ 的输出结果从左至右的顺序查找的
#如果在当前类中找到方法 就直接执行 不再搜索 如果没有找到 就查找下一个类中是否有对应的方法 如果找到 就直接执行不再搜索 如果找到最后一个类都没有找到方法 程序报错


# 多态
# 多态不同的子类对象 调用相同的父类方法 产生不同的执行结果
# 多态可以增加代码的灵活度
# 以继承和重写父类方法为前提 是调用方法的技巧 不会影响到类的内部设计 也就是说在不同子类中重写方法
#比如人类是父类 程序员和设计师是子类 子类在调用工作时会进行不同的动作 一个编程 一个画图

# 使用面向对象开发 第一步是设计类 -->使用 类名() 创建对象的动作有两步
#在内存中为对象分配空间
#调用初始化方法 __init__ 为对象初始化 
#对象创建后 内存中就有一个对象的实实在在的存在 -- 实例
# 通常会把 创建出来的对象叫做类的实例
#         创建对象的动作叫做实例化
#         对象的属性叫做实例属性
#         对象调用的方法叫做实例方法
...
#obj1 = AAA() 属于实例对象
# 在程序运行时 类同样会被加载到内存
#在Python中 类是一个特殊的对象 -- 类对象  在程序运行时 类对象在内存中只有一份 使用一个类可以创建出很多个对象实例 除了封装实例的属性和方法外 类对象还可以拥有自己的属性和方法
# 类属性
# 类方法
# 通过 类名. 的方式就可以访问类的属性或者类的方法
#
#类属性就是给类对象中定义的属性 同常用于记录与这个类相关的特征
#类属性不会用于记录具对象的特征

# 类方法和静态方法
# 类属性就是针对类对象定义的属性
# 使用赋值语句在 class关键字下方可以定义类属性  类属性用来记录与这个类相关的特征
# 类方法就是针对类对象定义的方法  在类对象内部可以直接访问或者调用其他的类方法
# 语法如下
@classmethod  #类方法需要一个修饰器@classmethod来标识 告诉解释器这是一个类方法
def 类方法名(cls):   #类方法的第一个参数应该是cls 类似于self
    pass
# 由哪一个类调用的方法 方法内部的cls就是哪一个类的引用  这个参数和实例方法的第一个参数self类似   使用其他的名称也可以 不过一般都是用的cls
# 通过 类名. 调用类方法 调用方法时 不需要传递cls参数
# 在方法内部 可以通过 cls. 访问类的属性
#         也可以通过 cls. 调用其他的类方法

# 静态方法
# 在开发时 如果需要在类中封装一个方法 这个方法即不需要访问实例属性或调用实例方法 也不需要访问类属性和调用类方法 这个时候 可以把这个方法封装成一个静态方法
# 语法如下:
@staticmethod
def 静态方法名():
    pass
# 和类方法一样 静态方法也需要一个修饰器@staticmethod来标识 告诉解释器这是一个静态方法
# 通过类名. 调用静态方法

# 单例设计模式
# 目的--让类创建的对象 在系统中只有唯一一个实例  每一次执行类名() 返回的对象 内存地址是相同的
# __new__方法
# 使用类名() 创建对象时 Python的解释器首先会调用__new__方法为对象分配空间  __new__是一个由object基类提供的内置的静态方法 主要作用有两个
# 在内存中为对象分配空间   返回对象的引用
# Python的解释器获得对象的引用后 将引用作为第一个参数传递给__init__方法
#
# 重写__new__方法的代码非常固定
...
#
# 如何只进行一次初始化工作
# 在每次使用类名() 创建对象时,Python的结束器都会自动调用两个方法 : __new__分配空间    __init__对象初始化
...
player2 = MusicPlayer()
print(player2)
# 在上面的的代码中对__new__方法进行改造之后 每次都会得到第一次被创建对象的引用 但是:初始化方法还会被再次使用
# 要求:初始化方法只被执行一次
解决办法:
